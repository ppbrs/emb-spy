...
def test_stm32h743_mmap() -> None:
    ...
    for adc_i in ["ADC1", "ADC2", "ADC3"]:
        names.extend([
            f"{adc_i}_CR", f"{adc_i}_CFGR", f"{adc_i}_CFGR2", f"{adc_i}_PCSEL",
            f"{adc_i}_SMPR1", f"{adc_i}_SMPR2",
            f"{adc_i}_SQR1", f"{adc_i}_SQR2", f"{adc_i}_SQR3", f"{adc_i}_SQR4",
        ])
    #   common
    for adc_c in ["ADC12", "ADC3"]:
        names.extend([f"{adc_c}_CSR", f"{adc_c}_CCR", f"{adc_c}_CDR", f"{adc_c}_CDR2", ])
    # ----------------------------------------------------------------------------------------------
    # BDMA
    ...
    names.extend(["DMAMUX1_CSR", "DMAMUX2_CSR", ])
    # ----------------------------------------------------------------------------------------------
    # MPU
