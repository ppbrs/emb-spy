...
def test_bits() -> None:
    ...
    assert Bits(bits=[30, 31], name="n").get_value(0b11000000_00000000_00000000_00000000) == 3
...
def test_bits_mask() -> None:
    ...
    assert Bits(bits=0, name="n").get_mask(invert=True) == 0b11111111_11111111_11111111_11111110
    assert (
        Bits(bits=[0, 31], name="n").get_mask(invert=True) == 0b01111111_11111111_11111111_11111110
    )


def test_bits_modify() -> None:
    """Test get_reg_value method."""
    assert Bits(bits=0, name="n").get_reg_value(reg_value=0b10101010, bits_value=0) == 0b10101010
    assert Bits(bits=0, name="n").get_reg_value(reg_value=0b01010100, bits_value=1) == 0b01010101
    assert Bits(bits=0, name="n").get_reg_value(reg_value=0b11111111, bits_value=0) == 0b11111110
    assert Bits(bits=0, name="n").get_reg_value(reg_value=0b00000001, bits_value=1) == 0b00000001

    assert (
        Bits(bits=[0, 7], name="n").get_reg_value(reg_value=0b11111111, bits_value=0) == 0b01111110
    )
    assert (
        Bits(bits=[0, 7], name="n").get_reg_value(reg_value=0b11111111, bits_value=1) == 0b01111111
    )
    assert (
        Bits(bits=[0, 7], name="n").get_reg_value(reg_value=0b11111111, bits_value=2) == 0b11111110
    )
    assert (
        Bits(bits=[0, 7], name="n").get_reg_value(reg_value=0b11111111, bits_value=3) == 0b11111111
    )
...
def test_bits_descr() -> None:
    ...
    short = obj.get_descr(
        reg_value=val1.to_bytes(length=1, byteorder="little", signed=False), verbose=False
    )
    verbose = obj.get_descr(
        reg_value=val2.to_bytes(length=1, byteorder="little", signed=False), verbose=True
    )
    assert short == "n = 0x1 = 0b01 = 1u."
    assert verbose == "n = 0x2 = 0b10 = 2u. descr, @[0,1].\n2 = two"
