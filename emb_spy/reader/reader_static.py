...
class ReaderStatic(_Reader):
    ...
    def read(self) -> dict[str, ReaderStatic]:
        ...
        self.logger.debug("mem_map before:")
        for addr, data in self.mem_map.items():
            self.logger.debug("  0x%08X: %s", addr, data.hex())
        ...
        self.logger.debug("mem_map after:")
        for addr, data in self.mem_map.items():
            self.logger.debug("  0x%08X: %s", addr, data.hex())
        ...
        for cfg in self.config_symbol:
            ...
            result = ReaderStaticResult(
                name=f"{cfg.name}, at 0x{cfg.mem_addr:08x}", val=val, raw=mem_data, descr=None
            )
        ...
        for cfg in self.config_memory:
            ...
            mem_data = self.mem_map[mem_addr]
            val = cfg.ctype.from_buffer_copy(mem_data)
            result = ReaderStaticResult(name="TBD", val=val, raw=mem_data, descr=None)
