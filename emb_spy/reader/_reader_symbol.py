...
from emb_spy.reader._reader_common import Name


@dataclasses.dataclass
class ReaderConfigSymbol:
    """
...
class _ReaderSymbol:
    ...
    def parse_config(
        ...
        if len(self.config_symbol):
            ...
            elf_symbols_map = Elf(elf_path=self.elf_path).get_symbols_map()
            for idx, cfg in enumerate(self.config_symbol):
                if cfg.name in elf_symbols_map:
                    self.config_symbol[idx].found_in_elf = True
                    self.logger.debug("Found `%s` in ELF.", cfg.name)
                    mem_data_size = elf_symbols_map[cfg.name].size  # Number of bytes occupied.
                    ...
                    mem_addr = elf_symbols_map[cfg.name].addr
