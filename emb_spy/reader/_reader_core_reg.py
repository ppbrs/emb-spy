    ...
    def parse_config(
        ...
        for idx, cfg in enumerate(self.config_core_reg):
            for name, register in self.soc.map_name().items():
                ...
                raise ValueError(f"Core register `{cfg.name}` is not in the provided SoC object.")
        ...
        for idx, cfg in enumerate(self.config_core_reg_bits):
            ...
            for name, register in self.soc.map_name().items():
                ...
                raise ValueError(f"Core register `{reg_name}` is not in the provided SoC object.")
