...
class Backend:
    ...
    def find_openocd_telnet_port() -> int:
        ...
        for proc in psutil.process_iter(["cmdline"]):
            ...
            if cmd_line is not None and len(cmd_line) > 1 and "openocd" in cmd_line[0]:
                logging.debug(
                    'Found OpenOCD process, pid=%d, cmd_line="%s".', proc.pid, " ".join(cmd_line)
                )
                ...
                port = 4444
                logging.warning(
                    "Telnet port was not provided explicitly, assuming the default "
                    "value %d is used.",
                    port,
                )
                ...
                try:
                    ...
                    logging.debug(
                        "Could not open TCP port %d. It is either used by another"
                        "application or not open at all.",
                        port,
                    )
    ...
    def __enter__(self):
        ...
        if self.target_name is not None:
            if self.target_name not in target_names:
                raise ValueError(
                    f"Target name `{self.target_name}` is not in the list of targets: {target_names}."
                )
    ...
    def request(self, cmd: str, timeout: float = 2.0) -> list[str]:
        ...
        * ">", which is the prompt for the following request.
        ...
        if r_lines[-1] != ">":
            self.logger.warning(
                "%s: Expected to receive `>` in the last line, but got `%s`", cmd, {r_lines[-1]}
            )
            r_lines = r_lines[1:0]
        else:
            r_lines = r_lines[1:-1]
    ...
    def read_core_register(self, name: str) -> bytes | None:
        """
        ...
        lines = [
            ...
            for line in rx_bytes.replace(b"\r", b"\n").split(b"\n")
            if line and line != b">"
        ]
        ...
        line = lines[1]
        assert len(line_parts := line.split(":")) == 2, (
            f"Line '{line}' doesn't have exactly one colon."
        )
