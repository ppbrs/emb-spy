...
def init_gpio(self: SoC, prefix: str, base: int) -> None:
    ...
    self.append(
        MmapReg(
            name=f"{prefix}_MODER",
            addr=(base + 0x00),
            descr="GPIO port mode register",
            bits=[
                Bits(
                    bits=range(2 * pin, 2 * pin + 2),
                    name=f"MODER{pin}",
                    descr_vals=moder_descr_vals,
                )
    ...
    self.append(
        MmapReg(
            name=f"{prefix}_OTYPER",
            addr=(base + 0x04),
            descr="GPIO port output type register",
            bits=[
                Bits(bits=range(pin, pin + 1), name=f"OT{pin}", descr_vals=otyper_descr_vals)
                for pin in range(0, 16)
            ],
        )
    )
    ...
    self.append(
        MmapReg(
            name=f"{prefix}_BSRR",
            addr=(base + 0x18),
            descr="GPIO port bit set/reset register",
            bits=(
                [
                    Bits(
                        bits=pin,
                        name=f"BS{pin}",
                        descr=f"Port {prefix} set I/O pin {pin}",
                        descr_vals={},
                    )
                    for pin in range(0, 16)
                ]
    ...
    lckr_bits = [
        Bits(
            bits=pin,
            name=f"LCK{pin}",
            descr=f"Port {prefix} lock I/O pin {pin}",
            descr_vals=lck_descr_vals,
        )
        for pin in range(0, 16)
    ]
    lckr_bits.append(
    ...
    self.append(
        MmapReg(
            ...
            bits=[
                Bits(bits=range(4 * pin, 4 * pin + 4), name=f"AFR{pin}", descr_vals=afr_descr_vals)
    ...
    self.append(
        MmapReg(
            ...
            bits=[
                Bits(
                    bits=range(4 * pin, 4 * pin + 4),
                    name=f"AFR{pin + 8}",
                    descr_vals=afr_descr_vals,
                )
                for pin in range(0, 8)
            ],
        )
