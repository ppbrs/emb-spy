...
def init_scb(self) -> None:
    ...
    self.append(
        MmapReg(
            name="SCB_CCR",
            addr=0xE000ED14,
            descr="Configuration and Control Register",
            bits=[
                Bits(bits=17, name="IC", descr="Enables L1 instruction cache."),
                Bits(bits=16, name="DC", descr="Enables L1 data cache."),
            ],
        )
    )
    ...
    self.append(
        MmapReg(
            name="SCB_SHPR1",
            addr=0xE000ED18,
            descr="System Handler Priority Register 1",
            bits=[
                Bits(
                    bits=range(16, 24),
                    name="PRI_6",
                    descr="Priority of system handler 6, UsageFault",
                ),
                Bits(
    ...
    self.append(
        MmapReg(
            name="SCB_SHPR2",
            addr=0xE000ED1C,
            descr="System Handler Priority Register 2",
            bits=[
                Bits(
                    bits=range(24, 32), name="PRI_11", descr="Priority of system handler 11, SVCall"
                ),
            ],
        )
    )
    self.append(
        MmapReg(
            name="SCB_SHPR3",
            addr=0xE000ED20,
            descr="System Handler Priority Register 3",
            bits=[
                Bits(
                    bits=range(24, 32),
                    name="PRI_15",
                ...
                Bits(
                    bits=range(16, 24), name="PRI_14", descr="Priority of system handler 14, PendSV"
                ),
            ],
        )
    )
    self.append(
        MmapReg(
            name="SCB_SHCSR",
            addr=0xE000ED24,
            descr="System Handler Control and State Register",
            bits=[
                Bits(
                    bits=18, name="USGFAULTENA", descr="UsageFault enable bit, set to 1 to enable"
                ),
                Bits(bits=17, name="BUSFAULTENA", descr="BusFault enable bit, set to 1 to enable"),
                Bits(bits=16, name="MEMFAULTENA", descr="MemManage enable bit, set to 1 to enable"),
                Bits(
                    bits=15,
                    name="SVCALLPENDED",
    ...
    self.append(
        MmapReg(
            name="SCB_CFSR",
            addr=0xE000ED28,
            descr="Configurable Fault Status Register",
            ...
            bits=[
                ...
                Bits(
                    bits=range(0, 8),
                    name="MMFSR",
                    descr="Memory Management Fault Status Register.",
                ),
                Bits(
                    bits=7,
                    name="MMFSR:MMARVALID",
                ...
                Bits(
                    bits=1,
                    name="MMFSR:DACCVIOL",
                    descr="Data access violation flag",
                ),
                ...
                Bits(
                    bits=0,
                    name="MMFSR:IACCVIOL",
                    descr="Instruction access violation flag",
                    descr_vals={
                ...
                Bits(
                    bits=range(8, 16),
                    name="BFSR",
                    descr="Bus Fault Status Register",
                ),
                Bits(
                    bits=(8 + 7),
                    name="BFSR:BFARVALID",
                    descr="Address Register (BFAR) valid flag",
                ),
                Bits(
                    bits=(8 + 5),
                    name="BFSR:LSPERR",
    ...
    self.append(
        MmapReg(
            name="SCB_HFSR",
            addr=0xE000ED2C,
            descr="HardFault Status Register",
    ...
    self.append(
        MmapReg(
            name="SCB_CPACR",
            addr=0xE000ED88,
            descr="Coprocessor Access Control Register",
