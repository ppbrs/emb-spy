...
class ARMV7EM(SoC):
    ...
    def init_core_registers(self) -> None:
        ...
        for i in range(13):
            self.append(CoreReg(name=f"R{i}", addr=f"r{i}", descr=f"General purpose register {i}."))
        ...
        self.append(
            CoreReg(
                name="PSR",
                addr="xPSR",
                descr="(Combined) Program Status Register",
                bits=[
                    ...
                    Bits(
                        bits=list(range(10, 16)) + list(range(25, 27)),
                        name="EPSR:ICI/IT",
        ...
        self.append(
            CoreReg(
                name="BASEPRI",
                addr="faultmask",
                descr="Base Priority Mask Register",
                bits=[
                    Bits(
                        bits=range(0, 8),
                        name="BASEPRI",
                        descr="Priority mask bits",
                    )
                ],
            )
        )
            ...
            CoreReg(
                ...
                bits=[
                    Bits(
                        bits=0,
                        name="nPRIV",
                        descr="Defines the Thread mode privilege level",
                        descr_vals={0: "Privileged", 1: "Unprivileged"},
                    ),
                    Bits(
                        bits=1,
                        name="SPSEL",
                        descr="Defines the current stack",
                        descr_vals={
