...
def init_debug(self):
    ...
    self.append(
        MmapReg(
            name="DWT_PCSR",
            addr=dwt_base + 0x01C,
            descr="DWT program counter sample register",
            bits=[Bits(bits=range(0, 32), name="EIASAMPLE")],
        )
    )
    # ======================================================================================
    itm_base = 0xE0000000
    self.append(
        MmapReg(
            name="ITM_TER",
            addr=itm_base + 0xE00,
            descr="ITM trace enable register",
            bits=[Bits(bits=range(0, 13), name="PRESCALER", descr="SWO baud rate scaling.")],
        )
    )
    for i in range(8):
        self.append(
            MmapReg(
                name=f"ITM_STIM{i}",
                addr=itm_base + i * 4,
                descr=f"ITM stimulus register {i}",
                bits=[],
            )
        )
    # ======================================================================================
    ...
    self.append(
        MmapReg(
            name="SWO_CODR",
            addr=swo_base + 0x010,
            descr="SWO current output divisor register",
            bits=[Bits(bits=range(0, 13), name="PRESCALER", descr="SWO baud rate scaling.")],
        )
    )
    self.append(
        MmapReg(
            name="SWO_SPPR",
            addr=swo_base + 0x0F0,
            descr="SWO selected pin protocol register",
            bits=[Bits(bits=[0, 1], name="PPROT", descr="Pin protocol.")],
        )
    )
    self.append(
        MmapReg(
            name="SWO_FFSR",
            addr=swo_base + 0x300,
            descr="SWO formatter and flush status register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_CLAIMSET",
            addr=swo_base + 0xFA0,
            descr="SWO claim tag set register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_CLAIMCLR",
            addr=swo_base + 0xFA4,
            descr="SWO claim tag clear register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_LAR",
            addr=swo_base + 0xFB0,
            descr="SWO lock access register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_LSR",
            addr=swo_base + 0xFB4,
            descr="SWO lock status register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_AUTHSTAT",
            addr=swo_base + 0xFB8,
            descr="SWO authentication status register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_DEVID",
            addr=swo_base + 0xFC8,
            descr="SWO device configuration register",
        )
    )
    self.append(
        MmapReg(
            name="SWO_DEVTYPE",
            addr=swo_base + 0xFCC,
            descr="SWO device type identifier register",
        )
    )
    self.append(
        MmapReg(
    ...
    self.append(
        MmapReg(
            name="SWTF_CTRL",
            addr=swtf_base + 0x000,
            descr="SWTF control register",
            bits=[
    ...
    self.append(
        MmapReg(
            name="SWTF_PRIORITY",
            addr=swtf_base + 0x004,
            descr="SWTF priority register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_CLAIMSET",
            addr=swtf_base + 0xFA0,
            descr="SWTF claim tag set register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_CLAIMCLR",
            addr=swtf_base + 0xFA4,
            descr="SWTF claim tag clear register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_LAR",
            addr=swtf_base + 0xFB0,
            descr="SWTF lock access register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_LSR",
            addr=swtf_base + 0xFB4,
            descr="SWTF lock status register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_AUTHSTAT",
            addr=swtf_base + 0xFB8,
            descr="SWTF authentication status register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_DEVID",
            addr=swtf_base + 0xFC8,
            descr="SWTF CoreSight device identity register",
        )
    )
    self.append(
        MmapReg(
            name="SWTF_DEVTYPE",
            addr=swtf_base + 0xFCC,
    ...
    self.append(
        MmapReg(
            name="DBGMCU_IDC",
            addr=dbgmcu_base + 0x000,
            descr="DBGMCU identity code register.",
            bits=[
    ...
    self.append(
        MmapReg(
            name="DBGMCU_CR",
            addr=dbgmcu_base + 0x004,
            descr="DBGMCU configuration register.",
            bits=[
