        ...
        if irq % 32 == 0:
            self.append(
                MmapReg(
                    ...
                    bits=[
                        Bits(bits=n, name=f"SETENA{irq + n}") for n in range(32) if irq + n < 240
                    ],
                )
            )
            self.append(
                MmapReg(
                    ...
                    bits=[
                        Bits(bits=n, name=f"CLRENA{irq + n}") for n in range(32) if irq + n < 240
                    ],
                )
            )
            self.append(
                MmapReg(
                    ...
                    bits=[
                        Bits(bits=n, name=f"SETPEND{irq + n}") for n in range(32) if irq + n < 240
                    ],
                )
            )
            self.append(
                MmapReg(
                    ...
                    bits=[
                        Bits(bits=n, name=f"CLRPEND{irq + n}") for n in range(32) if irq + n < 240
                    ],
                )
            )
            self.append(
                MmapReg(
                    ...
                    bits=[
                        Bits(bits=n, name=f"ACTIVE{irq + n}") for n in range(32) if irq + n < 240
                    ],
                )
            )
