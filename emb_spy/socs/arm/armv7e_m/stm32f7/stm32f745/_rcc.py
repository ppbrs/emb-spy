...
def init_rcc(self: SoC) -> None:
        ...
        MmapReg(
            ...
            bits=[
                ...
                Bits(
                    bits=25,
                    name="PLLRDY",
                    descr="Main PLL (PLL) clock ready flag",
                ),
                Bits(
                    bits=24,
                    name="PLLON",
                    descr="Main PLL (PLL) enable",
                ),
                Bits(
                    bits=19,
                    name="CSSON",
                    descr="Clock security system enable",
                ),
                Bits(
                    bits=18,
                    name="HSEBYP",
                ...
                Bits(
                    bits=range(8, 16), name="HSICAL", descr="Internal high-speed clock calibration"
                ),
        ...
        MmapReg(
            ...
            bits=[
                Bits(
                    bits=range(24, 28),
                    name="PLLQ",
                    descr="Main PLL (PLL) division factor for USB OTG FS, SDMMC and random number generator clocks",
                ),
                Bits(
                    bits=22,
                    name="PLLSRC",
                    descr="Main PLL(PLL) and audio PLL (PLLI2S) entry clock source",
                ),
                ...
                Bits(
                    bits=range(16, 18),
                    name="PLLP",
                    descr="Main PLL (PLL) division factor for main system clock",
                ),
                Bits(
                    bits=range(6, 15),
                    name="PLLN",
                    descr="Main PLL (PLL) multiplication factor for VCO",
                ),
                Bits(
                    bits=range(0, 6),
                    name="PLLM",
        ...
        MmapReg(
            ...
            bits=[
                Bits(
                    bits=range(30, 32),
                    name="MCO2",
                    descr="Microcontroller clock output 2",
                ),
                ...
                Bits(
                    bits=range(21, 23),
                    name="MCO1",
                    descr="Microcontroller clock output 1",
                ),
                ...
                Bits(
                    bits=range(2, 4),
                    name="SWS",
                    descr="System clock switch status",
                ),
    ...
    self.append(
        MmapReg(name="RCC_CIR", addr=(base + 0x0C), descr="RCC clock interrupt register", bits=[])
    )
    # ----------------------------------------------------------------------------------------------
    self.append(
        MmapReg(
    ...
    self.append(
        MmapReg(
            name="RCC_BDCR", addr=(base + 0x70), descr="RCC backup domain control register", bits=[]
        )
    )
    self.append(
        MmapReg(
            name="RCC_CSR", addr=(base + 0x74), descr="RCC clock control & status register", bits=[]
        )
    )
    self.append(
        MmapReg(
