...
def init_dma(self: SoC):
    ...
    for dma_idx in (1, 2):
        ...
        for stream_idx in range(8):
            self.append(
                MmapReg(
                    name=f"{dma}_S{stream_idx}CR",
                    addr=(base + 0x010 + 0x18 * stream_idx),
                    ...
                    bits=[
                        ...
                        Bits(
                            bits=20,
                            name="TRBUFF",
                            descr="Enable the DMA to handle bufferable transfers",
                        ),
                        # 0: Bufferable transfers not enabled
                        # 1: Bufferable transfers enabled
                        Bits(
                            bits=19, name="CT", descr="Current target (only in double-buffer mode)"
                        ),
                        ...
                        Bits(
                            bits=0, name="EN", descr="Stream enable/flag stream ready when read low"
                        ),
                    ],
                )
            )
            self.append(
                MmapReg(
                    name=f"{dma}_S{stream_idx}NDTR",
                    addr=(base + 0x014 + 0x18 * stream_idx),
            ...
            self.append(
                MmapReg(
                    name=f"{dma}_S{stream_idx}PAR",
                    addr=(base + 0x018 + 0x18 * stream_idx),
            ...
            self.append(
                MmapReg(
                    name=f"{dma}_S{stream_idx}M0AR",
                    addr=(base + 0x01C + 0x18 * stream_idx),
            ...
            self.append(
                MmapReg(
                    name=f"{dma}_S{stream_idx}M1AR",
                    addr=(base + 0x020 + 0x18 * stream_idx),
            ...
            self.append(
                MmapReg(
                    name=f"{dma}_S{stream_idx}FCR",
                    addr=(base + 0x024 + 0x18 * stream_idx),
