...
def init_pwr(self: SoC) -> None:
    ...
    self.append(
        MmapReg(
            name="PWR_CSR1",
            addr=(pwr_base + 0x004),
            descr="PWR control status register 1",
        )
    )
    self.append(
        MmapReg(
        ...
        MmapReg(
            ...
            bits=[
                ...
                Bits(
                    bits=2,
                    name="SCUEN",
                    descr="Supply configuration update enable",
                    descr_vals={
                ...
                Bits(
                    bits=1,
                    name="LDOEN",
                    descr="Low drop-out regulator enable",
                    descr_vals={
                ...
                Bits(
                    bits=0,
                    name="BYPASS",
                    descr="Power management unit bypass",
                    descr_vals={
    ...
    self.append(
        MmapReg(
            name="PWR_CPUCR",
            addr=(pwr_base + 0x010),
            descr="PWR CPU control register",
        )
    )
    self.append(
        MmapReg(
            name="PWR_D3CR",
            addr=(pwr_base + 0x018),
            descr="PWR D3 domain control register",
        )
    )
    self.append(
        MmapReg(
            name="PWR_WKUPCR",
            addr=(pwr_base + 0x020),
            descr="PWR wakeup clear register",
        )
    )
    self.append(
        MmapReg(
            name="PWR_WKUPFR",
            addr=(pwr_base + 0x024),
            descr="PWR wakeup flag register",
        )
    )
    self.append(
        MmapReg(
