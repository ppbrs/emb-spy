...
def _init_individual(self: SoC, idx: int, base: int) -> None:
    ...
    bits_cr = [
        ...
        Bits(bits=range(8, 10), name="BOOST", descr="Boost mode control",),
    ...
    self.append(MmapReg(
        name=f"{adc}_CR", addr=(base + 0x08),
        descr="ADC1 control register", bits=bits_cr,
    ))
    # ----------------------------------------------------------------------------------------------
    self.append(MmapReg(
        name=f"{adc}_CFGR", addr=(base + 0x0C),
        descr="ADC configuration register", bits=[
            Bits(
                bits=range(5, 10), name="EXTSEL",
            ...
            Bits(
                bits=range(10, 12), name="EXTEN",
                descr="External trigger enable and polarity selection for regular channel."),
            Bits(bits=range(2, 5), name="RES", descr="Data resolution."),
            # 000: 16 bits
            ...
            Bits(bits=range(0, 2), name="DMNGT", descr="Data Management configuration."),
    ...
    # ----------------------------------------------------------------------------------------------
    self.append(MmapReg(
        name=f"{adc}_CFGR2", addr=(base + 0x10),
        descr="ADC configuration register 2", bits=[
    ...
    bits_smpr1 = [
        Bits(
            ...
            descr_vals=bits_smpr_values) for ch in range(0, 10)
    ]
    self.append(MmapReg(
        name=f"{adc}_SMPR1", addr=(base + 0x14),
    ...
    bits_smpr2 = [
        Bits(
            ...
            descr=f"Channel {ch} sampling time selection",
            descr_vals=bits_smpr_values) for ch in range(10, 20)
    ]
    self.append(MmapReg(
        name=f"{adc}_SMPR2", addr=(base + 0x18),
        descr="ADC1 sample time register 2",
        bits=bits_smpr2,
    ))
    # ----------------------------------------------------------------------------------------------
    self.append(MmapReg(
        name=f"{adc}_PCSEL", addr=(base + 0x1C),
        descr="ADC channel preselection register", bits=[
    ...
    # ----------------------------------------------------------------------------------------------
    self.append(MmapReg(
        name=f"{adc}_SQR1", addr=(base + 0x30),
    ...
    self.append(MmapReg(
        name=f"{adc}_SQR2", addr=(base + 0x34), descr="ADC1 regular sequence register 2",
        bits=[
            ...
            Bits(bits=range(12, 16 + 1), name="SQ7", descr="7th conversion in regular sequence"),
            Bits(bits=range(18, 22 + 1), name="SQ8", descr="8th conversion in regular sequence"),
            Bits(bits=range(24, 28 + 1), name="SQ9", descr="9th conversion in regular sequence"),

        ],
    ))
    self.append(MmapReg(
        name=f"{adc}_SQR3", addr=(base + 0x38), descr="ADC1 regular sequence register 3",
        bits=[
            Bits(bits=range(0, 4 + 1), name="SQ10", descr="10th conversion in regular sequence"),
            Bits(bits=range(6, 10 + 1), name="SQ11", descr="11th conversion in regular sequence"),
            Bits(bits=range(12, 16 + 1), name="SQ12", descr="12th conversion in regular sequence"),
            Bits(bits=range(18, 22 + 1), name="SQ13", descr="13th conversion in regular sequence"),
            Bits(bits=range(24, 28 + 1), name="SQ14", descr="14th conversion in regular sequence"),
        ],
    ))
    self.append(MmapReg(
        name=f"{adc}_SQR4", addr=(base + 0x3C), descr="ADC1 regular sequence register 4",
        bits=[
            Bits(bits=range(0, 4 + 1), name="SQ15", descr="15th conversion in regular sequence"),
            Bits(bits=range(6, 10 + 1), name="SQ16", descr="16th conversion in regular sequence"),
        ],
    ))
    # ----------------------------------------------------------------------------------------------
...
def _init_common(self: SoC) -> None:
    ...
    ccr_bits = [
        ...
        Bits(
            bits=range(18, 22), name="PRESC", descr="ADC prescaler",
            descr_vals={
        ...
        Bits(
            bits=range(16, 18), name="CKMODE", descr="ADC clock mode",
            descr_vals={
    ...
    self.append(MmapReg(
        ...
        descr="ADC12 common control register", bits=ccr_bits,
    ))
    self.append(MmapReg(
        name="ADC3_CCR", addr=(adc3_common_base + 0x08),
        descr="ADC3 common control register", bits=ccr_bits,
    ))

    self.append(MmapReg(
        name="ADC12_CDR", addr=(adc12_common_base + 0x0C),
        descr="ADC 12 common regular data register for dual mode"))
    self.append(MmapReg(
        name="ADC3_CDR", addr=(adc3_common_base + 0x0C),
        descr="ADC 3 common regular data register for dual mode"))

    self.append(MmapReg(
        name="ADC12_CDR2", addr=(adc12_common_base + 0x10),
        descr="ADC 12 common egular data register for 32-bit dual mode"))
    self.append(MmapReg(
        name="ADC3_CDR2", addr=(adc3_common_base + 0x10),
