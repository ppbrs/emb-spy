...
def init_i2c(self: SoC):
    ...
    for x, i2cx_base in enumerate(
        [self.I2C1_BASE, self.I2C2_BASE, self.I2C3_BASE, self.I2C4_BASE], start=1
    ):
        self.append(
            MmapReg(
                name=f"I2C{x}_CR1",
                addr=(i2cx_base + 0x00),
                descr=f"I2C{x} control register 1",
                bits=[
        ...
        self.append(
            MmapReg(
                name=f"I2C{x}_CR2",
                addr=(i2cx_base + 0x04),
                descr=f"I2C{x} control register 2",
                bits=[
                    Bits(bits=range(0, 10), name="SADD", descr="Slave address (master mode)"),
                    Bits(
                        bits=10,
                        name="RD_WRN",
                        descr="Transfer direction (master mode)",
                        descr_vals={
        ...
        self.append(
            MmapReg(
                name=f"I2C{x}_OAR1",
                addr=(i2cx_base + 0x08),
                descr=f"I2C{x} own address 1 register",
                bits=[],
            )
        )
        self.append(
            MmapReg(
                name=f"I2C{x}_OAR2",
                addr=(i2cx_base + 0x0C),
                descr=f"I2C{x} own address 2 register",
                bits=[],
            )
        )
        self.append(
            MmapReg(
                name=f"I2C{x}_TIMINGR",
                addr=(i2cx_base + 0x10),
                descr=f"I2C{x} timing register",
                bits=[
        ...
        self.append(
            MmapReg(
                name=f"I2C{x}_TIMEOUTR",
                addr=(i2cx_base + 0x14),
                descr=f"I2C{x} timeout register",
                bits=[],
            )
        )
        self.append(
            MmapReg(
                ...
                bits=[
                    ...
                    Bits(
                        bits=range(17, 24), name="ADDCODE", descr="Address match code (Slave mode)"
                    ),
                ],
            )
        )
