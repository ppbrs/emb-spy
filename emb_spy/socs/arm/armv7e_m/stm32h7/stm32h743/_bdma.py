...
def init_bdma(self: SoC):
    ...
    self.append(
        MmapReg(name="BDMA_ISR", addr=(base + 0x00), descr="BDMA interrupt status", bits=isr_bits)
    )
    for chnl in range(0, 8):
        self.append(
            MmapReg(
                name=f"BDMA_CCR{chnl}",
                addr=(base + 0x08 + 0x14 * chnl),
        ...
        self.append(
            MmapReg(
                name=f"BDMA_CNDTR{chnl}",
                addr=(base + 0x0C + 0x14 * chnl),
                descr=f"BDMA channel {chnl} number of data to transfer register",
            )
        )
        self.append(
            MmapReg(
                name=f"BDMA_CPAR{chnl}",
                addr=(base + 0x10 + 0x14 * chnl),
                descr=f"BDMA channel {chnl} peripheral address register",
            )
        )
        self.append(
            MmapReg(
                name=f"BDMA_CM0AR{chnl}",
                addr=(base + 0x14 + 0x14 * chnl),
                descr=f"BDMA channel {chnl} memory 0 address register",
            )
        )
        self.append(
            MmapReg(
                name=f"BDMA_CM1AR{chnl}",
                addr=(base + 0x18 + 0x14 * chnl),
