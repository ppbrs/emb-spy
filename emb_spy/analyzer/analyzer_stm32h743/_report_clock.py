...
def report_clock(
    ...
    if (pll_src := bits_data["RCC_PLLCKSELR.PLLSRC"].val) == 0:
        ...
        ref123_before_div = self.state.hsi_freq
    elif pll_src == 2:
        # HSE is send to PLLs.
        assert self.state.hse_freq is not None
        ref123_before_div = self.state.hse_freq
    else:
        raise NotImplementedError
    self.state.ref1_freq = ref123_before_div / bits_data["RCC_PLLCKSELR.DIVM1"].val
    self.state.ref2_freq = ref123_before_div / bits_data["RCC_PLLCKSELR.DIVM2"].val
    self.state.ref3_freq = ref123_before_div / bits_data["RCC_PLLCKSELR.DIVM3"].val

    for i in [1, 2, 3]:  # i is PLL index
        pll_on_rdy = bits_data[f"RCC_CR.PLL{i}ON"].val and bits_data[f"RCC_CR.PLL{i}RDY"].val
        ...
        if pll_on_rdy:
            ...
            md_file.new_line(f"* PLL{i} is off or not ready.")
