...
def _report_adc_common(
    ...
    if ckmode == 0:
        ker_in_div = {
            ...
            0b1001: 64,
            0b1010: 128,
            0b1011: 256, }[bits_data[f"{adcc}_CCR.PRESC"].val]
        md_file.new_line(f"Asynchronous clock mode, divided by {ker_in_div} in prescaler, "
                         f"then by {2 if is_rev_v else 1}.")

        setattr(
            self.state, adc_ker_freq_name,
...
def _report_adc_individual(
    ...
    def report_boost():
        ...
        if is_rev_v:
            boost_descr = {
                ...
                0b11: "can be used when 25.0 MHz < ADC clock ≤ 50.0 MHz", }[boost]
        else:
            boost_descr = {
                0: "used when ADC clock < 20 MHz to save power at lower clock frequency",
                1: "used when ADC clock > 20 MHz", }[boost]
    ...
    def report_enabled_channels():
        ...
        md_file.new_line()
        md_file.new_table(
            columns=len(legend), rows=(1 + len(table) // len(legend)),
            text_align="left", text=(legend + table), )
    ...
    def report_external_trigger():
        if (ext_enabled := bits_data[f"{adci}_CFGR.EXTEN"].val) == 0:
            ...
            edge_str = {
                0b01: "rising edge",
                0b10: "falling edge",
                0b11: "both rising and falling edges", }[ext_enabled]
            ...
            event_str = [
                ...
                "lptim1_out",
                "lptim2_out",
                "lptim3_out", ][event_idx]  # adc_ext_trg21..31 are Reserved.
            md_file.new_line(
