...
def get_bits_config(
    ...
    for gpio_idx in [
        ...
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_MODER.MODER{pin_idx}")
            for pin_idx in range(16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_PUPDR.PUPDR{pin_idx}")
            for pin_idx in range(16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_OTYPER.OT{pin_idx}")
            for pin_idx in range(16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_AFRL.AFR{pin_idx}")
            for pin_idx in range(8)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_AFRH.AFR{pin_idx}")
            for pin_idx in range(8, 16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_OSPEEDR.OSPEEDR{pin_idx}")
            for pin_idx in range(16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_IDR.IDR{pin_idx}")
            for pin_idx in range(16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_ODR.ODR{pin_idx}")
            for pin_idx in range(16)
        ]
        config += [
            ReaderConfigMmapRegBits(name=f"GPIO{gpio_idx}_LCKR.LCK{pin_idx}")
            for pin_idx in range(16)
        ]
